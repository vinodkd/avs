...
def _find_thumbnail(clip_id: str, mark_id: str) -> Path | None:
    """Return the per-mark thumbnail, or a numbered frame as fallback."""
    p = config.THUMB_DIR / clip_id / f"mark_{mark_id}.jpg"
    if p.exists():
        return p
    # fallback: any jpg in the clip thumb dir
    thumb_dir = config.THUMB_DIR / clip_id
    if thumb_dir.exists():
        frames = sorted(thumb_dir.glob("*.jpg"))
        if frames:
            return frames[0]
    return None
...
def _build_html(mark_groups: dict[str, list[Mark]], clips: dict[str, Clip]) -> str:
    ...
    for label, marks in mark_groups.items():
        ...
        for mark in marks:
            ...
            thumb_src = _thumb_b64(_find_thumbnail(mark.clip_id, mark.id))
