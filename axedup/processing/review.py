...
def open_review(session_id: str, console: Console | None = None) -> None:
    ...
    _log(f"Building review page for {len(marks)} candidate(s) …")
    html_bytes = _build_html(marks, clips).encode("utf-8")
...
def _build_html(marks: list[Mark], clips: dict[str, Clip]) -> str:
    cards_html = ""
    for mark in marks:
        clip = clips.get(mark.clip_id)
        mid = (mark.in_s + mark.out_s) / 2
        thumb_path = _find_thumbnail(mark.clip_id, mid)
        thumb_src = _thumb_b64(thumb_path)
        dur = mark.out_s - mark.in_s
        time_range = f"{_fmt_time(mark.in_s)} – {_fmt_time(mark.out_s)}"
        score_pct = int(mark.score * 100)
        ...
        <div class="card" id="card-{mark.id}" data-id="{mark.id}" data-accepted="true">
          ...
          <div class="meta">
            ...
            <div class="score-bar">
              ...
              <span class="score-label">{mark.score:.3f}</span>
            </div>
          </div>
          <div class="actions">
...
<div class="footer">
  <div class="summary">
    <span id="acc-count">{len(marks)}</span> accepted &nbsp;·&nbsp;
...
<script>
  ...
  function toggle(id, accepted) {{
    ...
    card.dataset.accepted = accepted;
